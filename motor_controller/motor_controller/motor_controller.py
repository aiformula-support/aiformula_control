...
class MotorController(Node):
    ...
    def __init__(self):
        ...
        self.frame_msg.id = 0x290  # MotorController CAN ID : 0x290
        self.frame_msg.dlc = 8  # Data length
        self.flag_msg = Frame()
        self.flag_msg.header.frame_id = "can0"     # Default can0
        self.flag_msg.id = 0x292                   # MotorController CAN ID : 0x210
    ...
    def toRefRPM(self, linear_velocity, angular_velocity):  # Calc Motor ref rad/s
        ...
        wheel_angular_velocities[DriveWheel.LEFT] = (linear_velocity / (self.diameter * 0.5)) - (
            self.tread / self.diameter
        ) * angular_velocity  # [rad/s]
        wheel_angular_velocities[DriveWheel.RIGHT] = (linear_velocity / (self.diameter * 0.5)) + (
            self.tread / self.diameter
        ) * angular_velocity  # [rad/s]
        minute_to_second = 60.0
        rpm = wheel_angular_velocities * (minute_to_second / (2.0 * np.pi))
