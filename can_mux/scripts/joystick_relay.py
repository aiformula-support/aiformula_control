...
from geometry_msgs.msg import Can
from rclpy.action import ActionServer
from rclpy.executors import ExternalShutdownException
from rclpy.qos import QoSProfile, QoSDurabilityPolicy
from rclpy.node import Node
from std_msgs.msg import Bool
from can_mux_msgs.action import JoyPriority, JoyTurbo
...
class VelocityControl:
    ...
    def scale_twist(self, cmd):
        ...
        if self.validate_twist(cmd):
            if cmd.linear.x >= 0:
                Can.linear.x = self._forward(
                    cmd.linear.x, self._current_step)
            else:
                Can.linear.x = self._backward(
                    cmd.linear.x, self._current_step)
            Can.linear.y = self._lateral(cmd.linear.y, self._current_step)
            Can.angular.z = self._angular(
...
class JoystickRelay(Node):
    ...
    def __init__(self):
        ...
        self._pub_cmd = self.create_publisher(Can, 'joy_vel_out', 1)
        self._subscriber = self.create_subscription(
            Can, 'joy_vel_in', self._forward_cmd, 1)
